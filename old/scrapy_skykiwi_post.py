...
def connect_db_read_post():
	...
	conn.commit()
	try:
		cur = cursor.execute("SELECT * FROM skykiwi_table WHERE published = No")
		# 获取数据库中表的全部数据
		data = cur.fetchall()
		if data:
			d = open_Driver()
			for row in data:
				...
				try:
					...
					try:
						cursor.execute("UPDATE skykiwi_table SET published = Yes WHERE url = '%s'" % url)
						conn.commit()
...
def post(d, url, title, date_publish, content):
	directory = r"D:\Desktop\img_skykiwi_news" + '\\' + url.split('/')[-1].split('.')[0]
	c = 1
	cc = 1
	...
	time.sleep(3)
	d.find_element_by_name('from').send_keys("天维网")
	time.sleep(3)
